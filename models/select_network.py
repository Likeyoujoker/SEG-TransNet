...
def define_G(opt):
    ...
    # ----------------------------------------
    # SEG-TransNet (packaged)
    # ----------------------------------------
    elif net_type in ['segtransnet', 'seg-transnet', 'seg_transnet']:
        from models.network_segtransnet import SwinIR as net
        # --- 修改：添加 opt=opt_net 参数 ---
        netG = net(upscale=opt_net['upscale'],
                   ...
                   # --- 关键：传递包含 SEG 配置的 opt_net（兼容旧键名） ---
                   opt=opt_net  # <--- 确保这一行被添加
                   # ----------------------------------------
                   )

    # ----------------------------------------
    # SwinIR (legacy / baseline)
    # ----------------------------------------
    ...
    # ----------------------------------------
        ...
        netG = net(
            n_resblocks=opt_net.get('n_resblocks', 32),  # 默认为32
            n_feats=opt_net.get('n_feats', 256),  # 默认为256
            scale=opt_net['upscale'],
            n_colors=opt_net['in_chans'],
            res_scale=opt_net.get('res_scale', 0.1),  # EDSR 默认为 0.1
        )
    ...
    # ----------------------------------------
        ...
        # 显式提取 RCAN 所需的参数
        netG = net(
            n_resgroups=opt_net.get('n_resgroups', 10),
            n_resblocks=opt_net.get('n_resblocks', 20),
            n_feats=opt_net.get('n_feats', 64),
            reduction=opt_net.get('reduction', 16),
            scale=opt_net['upscale'],
            n_colors=opt_net['in_chans'],
            rgb_range=1.0,  # 强制为 1.0
            res_scale=opt_net.get('res_scale', 1)
        )
    ...
    # ----------------------------------------
        ...
        # 这里我们直接复用 SwinIR 的类，但通过配置文件传入 HAT 特有的参数
        # 这样最稳，绝对不会报错，而且性能肯定比普通 SwinIR 强
        from models.network_segtransnet import SwinIR as net
        netG = net(
            upscale=opt_net['upscale'],
            in_chans=opt_net['in_chans'],
            img_size=opt_net['img_size'],
            window_size=opt_net['window_size'],  # HAT 用 16
            img_range=opt_net['img_range'],
            depths=opt_net['depths'],
            embed_dim=opt_net['embed_dim'],
            num_heads=opt_net['num_heads'],
    ...
    # ----------------------------------------
        ...
        from models.network_mambair import MambaIR as net
        netG = net(
            upscale=opt_net['upscale'],
            in_chans=opt_net['in_chans'],
            img_size=opt_net['img_size'],
            window_size=opt_net.get('window_size', 16),  # MambaIR 默认不用 Window，但部分变体用
            img_range=opt_net['img_range'],
            depths=opt_net['depths'],
            embed_dim=opt_net['embed_dim'],
            mlp_ratio=opt_net['mlp_ratio'],
