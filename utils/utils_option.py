...
def get_scheduler(opt, optimizer):
    """根据配置返回学习率调度器"""
    train_opt = opt['train']
    scheduler_type = train_opt.get('G_scheduler_type', None)

    if scheduler_type is None or scheduler_type == 'None':
        return None
    elif scheduler_type == 'MultiStepLR':
        milestones = train_opt['G_scheduler_milestones']
        gamma = train_opt['G_scheduler_gamma']
        return torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma)
    elif scheduler_type == 'CosineAnnealingLR':
        T_max = train_opt['G_scheduler_T_max']
        eta_min = train_opt.get('G_scheduler_eta_min', 0)
        return torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max, eta_min)
    elif scheduler_type == 'StepLR':
        step_size = train_opt['G_scheduler_step_size']
        gamma = train_opt['G_scheduler_gamma']
